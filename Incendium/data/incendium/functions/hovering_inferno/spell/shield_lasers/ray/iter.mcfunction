...
execute if score #hit in.fire_laser matches 0 positioned ~-0.05 ~-0.05 ~-0.05 as @e[type=player,tag=in.survival_player,tag=!laser,dx=0,sort=nearest,limit=1] positioned ~-0.85 ~-0.85 ~-0.85 if entity @s[dx=0] run function incendium:hovering_inferno/misc/generic_hit_entity
